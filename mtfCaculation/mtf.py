...
class MTF:
    ...
    def GetEdgeSpreadFunction(imgArr, edgePoly, verbose=Verbosity.NONE):
        ...
        values = np.reshape(imgArr, X*Y)#二维变成一维
        ...
        distances = np.reshape(distance, X*Y)#二维变成一维
        #print(distances)
        ...
        sign = 1
        if np.average(values[indexes[:10]]) > np.average(values[indexes[-10:]]):#如果distance越小的像素点的value值越大
            sign = -1
    ...
    def GetMTF(lsf, pixel_width,verbose=Verbosity.NONE):
        ...
        values = 1/np.sum(lsf.y)*abs(fft(lsf.y)) # 大小499
