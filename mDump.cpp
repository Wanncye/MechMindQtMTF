...
#define DEFAULT_ENGLISH_MESSAGE_TEXT                                                               \
    L"%s experienced an unknown error and had to exit. \nHowever, some error information has "     \
    L"been saved in %s. \nPlease, email this file to <hassan_deldar@yahoo.com> if you would like " \
    L"to help us debug the problem."
...
static int DUMP_TYPE_FULL = MiniDumpNormal | MiniDumpWithFullMemory | MiniDumpWithDataSegs |
...
LONG MiniDumper::TopLevelFilter(struct _EXCEPTION_POINTERS* pExceptionInfo)
{
    ...
    if (hDll) {
        ...
        if (pDump) {
            ...
            // if (::MessageBox( NULL, _T("Something bad happened in your program, would you like to
            // save a diagnostic file?"), m_szAppName, MB_YESNO )==IDYES)
            {
                ...
                if (hFile != INVALID_HANDLE_VALUE) {
                    ...
                    BOOL bOK = pDump(GetCurrentProcess(), GetCurrentProcessId(), hFile,
                                     (MINIDUMP_TYPE)DUMP_TYPE_FULL, &ExInfo, NULL, NULL);
                    if (bOK) {
                        swprintf(szScratch, sizeof(szScratch), L"Saved dump file to '%s'",
                                 szDumpPath);
                        szResult = szScratch;
                        retval = EXCEPTION_EXECUTE_HANDLER;
                    } else {
                        swprintf(szScratch, sizeof(szScratch),
                                 L"Failed to save dump file to '%s' (error %d)", szDumpPath,
                                 GetLastError());
                        szResult = szScratch;
                    }
                    CloseHandle(hFile);
                    ...
                    swprintf(csOutMessage, sizeof(csOutMessage), m_szMessageText, m_szAppName,
                             szDumpPath);
                    ...
                    swprintf(szScratch, sizeof(szScratch),
                             L"Failed to create dump file '%s' (error %d)", szDumpPath,
                             GetLastError());
                    szResult = szScratch;
                }
            }
